  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row gap-5 my-6 mx-3 md:mx-20">
          ...
          {cart && cart.length > 0 ? (
            cart.map((item, index) => {
              return (
                <div
                  key={item._id}
                  className="shadow-lg border-2 rounded-md flex flex-col md:flex-row items-start md:items-center justify-between p-3 gap-5 md:gap-2 bg-white"
                >
                  <Link
                    to={`/product/${item._id}`}
        ...
        <div className="border-2 border-gray-200 shadow-lg rounded-lg w-full md:w-[30%] h-fit p-8 bg-white">
